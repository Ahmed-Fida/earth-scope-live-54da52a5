...
function generateSO2(lat: number, lon: number, startYear: number, endYear: number) {
  ...
  let baseSO2 = isIndustrial ? 0.35 : 0.12; // DU (Dobson Units)

  for (let year = startYear; year <= endYear; year++) {
    for (let month = 1; month <= 12; month++) {
      const date = `${year}-${month.toString().padStart(2, '0')}-01`;
      const monthRad = ((month - 1) / 12) * 2 * Math.PI;
      const seasonal = 0.08 * Math.sin(monthRad + Math.PI); // Winter peak
      const random = (Math.random() - 0.5) * 0.06;
      let so2 = baseSO2 + seasonal + random;
  ...
  try {
    ...
    return new Response(JSON.stringify({
      ...
      insights: [
        ...
        'SO₂ peaks during winter due to increased fossil fuel combustion and lower atmospheric mixing.',
      ],
