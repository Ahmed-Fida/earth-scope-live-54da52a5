...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({
      ...
      stats: { mean: Number(mean.toFixed(4)), min: Number(min.toFixed(4)), max: Number(max.toFixed(4)), trend: Math.abs(trendPct) < 2 ? 'stable' : trendPct > 0 ? 'increasing' : 'decreasing', trendPercent: Number(trendPct.toFixed(1)) },
      insights: [
        mean > 2 ? 'High aerosol loading detected — likely dust or industrial pollution.' : mean > 1 ? 'Moderate aerosol levels — typical for semi-arid regions.' : 'Low aerosol index — clean atmospheric conditions.',
