...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({
      ...
      stats: { mean: Number(mean.toFixed(4)), min: Number(Math.min(...values).toFixed(4)), max: Number(Math.max(...values).toFixed(4)), peakMonth: 'December-January (Winter)', lowMonth: 'July-August (Monsoon)' },
